...
export class PlayerComponent {
  ...
  private playNextSongInQueue() {
    if (this.nextQueue.length > 0) {
      ...
      if (this.currentSong) {
        // We add the specified element to the beginning of the array.
        this.previousQueue.unshift(this.currentSong);
      }
      // We remove the first element from the array and returns that removed element.
  ...
  protected readonly onvolumechange = onvolumechange;
}

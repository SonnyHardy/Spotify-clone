...
import {SaveSong, TitleVO} from "../service/model/song.model";
...
export class AddSongComponent implements OnDestroy {
  ...
  constructor() {
    effect(() => {
      ...
      if (this.songService.addSig().status === "OK") {
        ...
        this.toastService.show('Error, please try again ❌', "DANGER");
      }
    });
  }

  ngOnDestroy(): void {
    this.songService.reset();   // we reset the signal
  }
  ...
  create(): void {
    ...
    const authorVO: TitleVO = {value: this.createForm.value.author};
